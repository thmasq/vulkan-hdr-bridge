...
static IS_RECORDING: std::sync::LazyLock<Arc<AtomicBool>> =
    std::sync::LazyLock::new(|| Arc::new(AtomicBool::new(false)));

static INSTANCES: std::sync::LazyLock<Mutex<HashMap<usize, InstanceState>>> =
    std::sync::LazyLock::new(|| Mutex::new(HashMap::new()));

static DEVICES: std::sync::LazyLock<Mutex<HashMap<usize, Arc<Mutex<DeviceState>>>>> =
    std::sync::LazyLock::new(|| Mutex::new(HashMap::new()));
...
const unsafe fn dispatch_key(handle: u64) -> usize {
...
pub unsafe extern "system" fn hdr_bridge_GetInstanceProcAddr(
    ...
    match name {
        b"vkGetInstanceProcAddr" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_GetInstanceProcAddr as *const () as usize,
                )
            };
        }
        b"vkCreateInstance" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_CreateInstance as *const () as usize,
                )
            };
        }
        b"vkDestroyInstance" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_DestroyInstance as *const () as usize,
                )
            };
        }
        b"vkCreateDevice" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_CreateDevice as *const () as usize,
                )
            };
        }
        b"vkGetDeviceProcAddr" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_GetDeviceProcAddr as *const () as usize,
                )
            };
        }
    ...
    if instance == vk::Instance::null() {
        return unsafe {
            std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(0usize)
        };
    }
    let key = unsafe { dispatch_key(instance.as_raw()) };

    INSTANCES.lock().unwrap().get(&key).map_or_else(
        || unsafe {
            std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(0usize)
        },
        |state| unsafe { (state.next_gipa)(instance, p_name) },
    )
}
...
pub unsafe extern "system" fn hdr_bridge_GetDeviceProcAddr(
    ...
    match name {
        ...
        b"vkCreateSwapchainKHR" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_CreateSwapchainKHR as *const () as usize,
                )
            };
        }
        b"vkDestroySwapchainKHR" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_DestroySwapchainKHR as *const () as usize,
                )
            };
        }
        b"vkQueuePresentKHR" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_QueuePresentKHR as *const () as usize,
                )
            };
        }
        b"vkGetDeviceQueue" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_GetDeviceQueue as *const () as usize,
                )
            };
        }
        b"vkGetDeviceQueue2" => {
            return unsafe {
                std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(
                    hdr_bridge_GetDeviceQueue2 as *const () as usize,
                )
            };
        }
        _ => {}
    ...
    DEVICES.lock().unwrap().get(&key).cloned().map_or_else(
        || unsafe {
            std::mem::transmute::<usize, std::option::Option<unsafe extern "system" fn()>>(0usize)
        },
        |arc| {
...
pub unsafe extern "system" fn hdr_bridge_CreateInstance(
    ...
    let Some(link) = (unsafe { find_instance_link((*p_create_info).p_next.cast()) }) else {
    ...
    unsafe { advance_instance_link((*p_create_info).p_next.cast_mut()) };

    let create_fn: PfnCreateInstance = unsafe {
        std::mem::transmute(next_gipa(
            vk::Instance::null(),
            c"vkCreateInstance".as_ptr().cast(),
        ))
    };
    ...
    log::info!("HDR Bridge: vkCreateInstance hooked (key={key:#x})");
    vk::Result::SUCCESS
}
...
pub unsafe extern "system" fn hdr_bridge_DestroyInstance(
    ...
    if let Some(gipa) = next_gipa {
        let destroy_fn: PfnDestroyInstance =
            unsafe { std::mem::transmute(gipa(instance, c"vkDestroyInstance".as_ptr().cast())) };
...
pub unsafe extern "system" fn hdr_bridge_GetDeviceQueue(
    ...
    let real_fn: unsafe extern "system" fn(vk::Device, u32, u32, *mut vk::Queue) =
        unsafe { std::mem::transmute(next_gdpa(device, c"vkGetDeviceQueue".as_ptr().cast())) };
...
pub unsafe extern "system" fn hdr_bridge_GetDeviceQueue2(
    ...
    let real_fn: unsafe extern "system" fn(
        ...
        *mut vk::Queue,
    ) = unsafe { std::mem::transmute(next_gdpa(device, c"vkGetDeviceQueue2".as_ptr().cast())) };
...
pub unsafe extern "system" fn hdr_bridge_CreateDevice(
    ...
    let Some(link) = (unsafe { find_device_link((*p_create_info).p_next.cast()) }) else {
    ...
    unsafe { advance_device_link((*p_create_info).p_next.cast_mut()) };

    let create_fn: PfnCreateDevice = unsafe {
        std::mem::transmute(next_gipa(
            vk::Instance::null(),
            c"vkCreateDevice".as_ptr().cast(),
        ))
    };
    ...
    let it = {
        ...
        instances.values().find(|_| true).map_or_else(
    ...
    spawn_socket_listener(recording_state, control_tx_clone);
    spawn_ffmpeg_sink(frame_rx, control_rx, output_base);
    ...
    log::info!("HDR Bridge: vkCreateDevice hooked (key={key:#x})");
    vk::Result::SUCCESS
}

fn spawn_ffmpeg_sink(
    frame_rx: crossbeam_channel::Receiver<CapturedFrame>,
    control_rx: crossbeam_channel::Receiver<()>,
    output_base: String,
) {
...
fn spawn_socket_listener(recording_state: Arc<AtomicBool>, control_tx_clone: Sender<()>) {
    std::thread::spawn(move || {
...
pub unsafe extern "system" fn hdr_bridge_DestroyDevice(
    ...
    if let Some(gdpa) = next_gdpa {
        let destroy_fn: PfnDestroyDevice =
            unsafe { std::mem::transmute(gdpa(device, c"vkDestroyDevice".as_ptr().cast())) };
...
pub unsafe extern "system" fn hdr_bridge_CreateSwapchainKHR(
    ...
    let value = DEVICES.lock().unwrap().get(&dev_key).cloned();
    ...
    let result = {
        let state = arc.lock().unwrap();
        unsafe {
            (state.dt.create_swapchain_khr)(
                device,
                &raw const modified_info,
                p_allocator,
                p_swapchain,
            )
        }
...
pub unsafe extern "system" fn hdr_bridge_DestroySwapchainKHR(
    ...
    if let Some(arc) = value {
        ...
        log::info!("HDR Bridge: swapchain {sc_key:#x} destroyed");
...
pub unsafe extern "system" fn hdr_bridge_QueuePresentKHR(
    ...
    let value = DEVICES.lock().unwrap().get(&queue_key).cloned();
    if let Some(arc) = value {
        ...
        return unsafe { (dt.queue_present_khr)(queue, &raw const next_present) };
    }

    log::error!("hdr_bridge_QueuePresentKHR: no device state for queue key {queue_key:#x}");
...
unsafe fn find_instance_link(mut p: *const c_void) -> Option<*mut VkLayerInstanceLink> {
    while !p.is_null() {
        let base = unsafe { &*p.cast::<vk::BaseInStructure>() };
        if base.s_type == VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO {
            let info = unsafe { &*p.cast::<VkLayerInstanceCreateInfo>() };
...
unsafe fn find_device_link(mut p: *const c_void) -> Option<*mut VkLayerDeviceLink> {
    while !p.is_null() {
        ...
        if base.s_type == VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO {
            let info = unsafe { &*p.cast::<VkLayerDeviceCreateInfo>() };
            if info.function == VK_LAYER_LINK_INFO && !info.u_layer_info.is_null() {
                return Some(info.u_layer_info);
            }
        }
...
unsafe fn advance_instance_link(mut p: *mut c_void) {
    while !p.is_null() {
        let base = unsafe { &mut *p.cast::<vk::BaseOutStructure>() };
        if base.s_type == VK_STRUCTURE_TYPE_LOADER_INSTANCE_CREATE_INFO {
            let info = unsafe { &mut *p.cast::<VkLayerInstanceCreateInfo>() };
...
unsafe fn advance_device_link(mut p: *mut c_void) {
    while !p.is_null() {
        let base = unsafe { &mut *p.cast::<vk::BaseOutStructure>() };
        if base.s_type == VK_STRUCTURE_TYPE_LOADER_DEVICE_CREATE_INFO {
            let info = unsafe { &mut *p.cast::<VkLayerDeviceCreateInfo>() };
            if info.function == VK_LAYER_LINK_INFO {
                info.u_layer_info = unsafe { (*info.u_layer_info).p_next };
                return;
            }
...
unsafe fn get_swapchain_images(
    ...
    let mut count = 0u32;
    (unsafe {
        (dt.get_swapchain_images_khr)(device, swapchain, &raw mut count, std::ptr::null_mut())
            .result()
    })?;
    let mut images = vec![vk::Image::null(); count as usize];
    (unsafe {
        (dt.get_swapchain_images_khr)(device, swapchain, &raw mut count, images.as_mut_ptr())
            .result()
    })?;
    Ok(images)
}
